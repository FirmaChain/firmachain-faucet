...
```javascript
FAUCET_MNEMONIC =
	'twist real vast bronze phrase impose million equip claim flight scatter embark nation thumb easy mother walk income talent better lesson horse arrest any';
```
...
// line - 367
{
	openRecaptcha && (
		<ReCaptchaBox>
			<ReCAPTCHA style={{ display: 'inline-block', height: '35px' }} theme="light" sitekey={reCaptchaSiteKey} onChange={handleRecaptcha} />
		</ReCaptchaBox>
	);
}
```
